...
class imageHandler {
  ...
  public imageHandler() {//HOW IT WORKS: we have an array for each cutscene full of numbers representing when the different images shoudld show up. Once a cutscene's time to appear happens,
    ...
    bgimg = loadImage("road1.png");//if images are used a lot (eg for things that are drawn constantly), we create a variable for them. Otherwise, we just load them as needed
    ...
    if (stage == MENU)//display backgrounds according to stage
      ...
      if (counter < introTimes.length && introTimes[counter]<=music.position()) {//INTRO cutscene. Runs until the index of the intro cutscene array goes out of bounds
        if (music.position() < 28476)//if position of music is less than a certain point, display one type of file
          background(loadImage("introCutscene"+int(counter+1)+".png"));//counter+1 because files start at "introCutscene1" instead of "introCutscene0"
        else//once it reaches past that point, show these other pictures
          background(loadImage("i"+int(counter-23)+"-01.jpg"));//counter-23 to account for the fact that the files are named starting at "1", but the counter is already at 24 at this point
        counter++;
      ...
      if (counter < bossTimes.length && bossTimes[counter]<=music.position()) {
        ...
        if (counter%6==0)
          background(loadImage("spinner9.png"));//alternate between two images
        else if (counter%6 == 3)
      ...
      if (stage == BOSSMAIN && music.position() < 25300) {
        if (bossCounter < bossCharge.length && bossCharge[bossCounter]<=music.position()) {
          enemyCarImg = loadImage("charge"+int(bossCounter)+".png");//charging-up animation for the fidget-spinner enemy during a certain part of the BOSSMAIN stage
          bossCounter++;
        }
      ...
      if (blueCar.streak>0) {
        image(loadImage("fire"+bg+".png"), 0, height-50);//if your "streak" stat > 0, display streak with a cool image (animated using the bg variable) behind it
